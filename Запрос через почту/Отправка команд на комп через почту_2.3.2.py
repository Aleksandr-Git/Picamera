# !!!!ДОБАВИТЬ КОММАНДУ НА ОТПРАВКУ ФОТО С КАМЕРЫ

# проверяет наличие новых писем
# проверяяет новое письмо на соответствие
# создает файл для хранения UID последнего писяма
# открывает новый поток и выполняет необходимую команду из письма
# если оттправить письмо тема:Связь, тель:Калькулятор, то откроется калькулятор
...
def new_email(last_uid):  # выполняет проверку письма на соответствие
    ...
    if SENDER in FROM[0] and SUBJECT == TEMA:  # если отпраывитель и тема совпадают
